/**
 * Repository-level proof that Retinue contract writes are SIGNED by the
 * connected wallet.
 *
 * Retinue's wallet context (lib/wallet.tsx) builds the client with the connected
 * wallet's EIP-1193 provider:
 *
 *   createClient({ chain: CHAIN, account, provider })
 *
 * and every exported action (acceptMandate, retainFromOffer, ...) routes through
 * an internal writeAndWait(client, ...) that calls client.writeContract. So the
 * signing request (eth_sendTransaction) goes through the wallet the user picked
 * — never genlayer-js's implicit window.ethereum fallback. These tests pin that
 * by feeding a provider-backed client into the real acceptMandate action.
 */
import { describe, it, expect, vi, beforeEach, afterEach } from "vitest";
import { createClient } from "genlayer-js";
import { studionet } from "genlayer-js/chains";
import { acceptMandate } from "../lib/retinue";
...
describe("Retinue writes are signed by the connected wallet provider", () => {
  it("routes writeContract through the injected EIP-1193 provider (correct from)", async () => {
    ...
    const txHash = await client.writeContract({
      address: CONTRACT, functionName: "accept_mandate", args: ["m-1"], value: 0n,
    });
